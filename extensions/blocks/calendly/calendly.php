	...
	if ( 'link' === $style ) {
		...
		if ( ! empty( $submit_button_text_color ) || ! empty( $submit_button_background_color ) ) {
			$inline_styles = sprintf(
				'.wp-block-jetpack-calendly .wp-block-button__link{%1$s%2$s}',
		...
		$content = sprintf(
			'<div class="wp-block-button %1$s"><a class="%2$s" role="button" onclick="Calendly.initPopupWidget({url:\'%3$s\'});return false;">%4$s</a></div>',
			esc_attr( $classes ),
